...
### 13.  发布后怎么做到秒级回滚且新旧产物不互相踩踏？ 

 产物带内容 hash 且新旧共存不覆盖，回滚只切 manifest/入口；版本轮询让长驻客户端感知切换（Kit 式 version poll 同理）；数据库迁移向前兼容（先加列后切码）是回滚可行性的隐藏前提。 

**来源**： https://github.com/solidjs/solid-start ； https://vitejs.dev/guide/build 

### 14.  边缘平台上 SSR 的性能预算怎么估？什么信号说明该退静态？ 

 冷启动时间+CPU 毫秒配额×请求量 vs 边缘函数单价；P50 渲染耗时逼近 TTFB 目标、或内容低变动时，预渲染/ISR 式方案完胜实时 SSR；用真实流量画像而不是感觉做决策。 

**来源**： https://docs.solidjs.com/ 

### 15.  灰度发布与错误监控在部署管线里的落地方案？ 

 边缘规则按 cookie/header 分流双版本，监控按版本维度打 tag 对比错误率与性能指标；放量步长与自动回滚阈值（错误率、5xx 比例）预设，发布单里写死观测窗口。 
