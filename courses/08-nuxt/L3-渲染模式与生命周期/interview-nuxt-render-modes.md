...
### D4.  同一业务页在 swr、isr、纯 SSR、prerender 四档之间的成本收益账怎么算？

**答**：先立坐标：代价=新鲜度损失+失效工程复杂度；收益=TTFB、源端负载、成本。逐档：prerender 零源端负载、秒开，但发布才更新、页面数进构建时长——适合改动以周为计；swr 构建解耦、页面无上限，代价是"多久旧"要人设计（maxAge 拍脑袋=事故），Node 自部署时缓存只在单实例，多实例=多份旧页并存，要配失效广播才完整；isr 把这套搬边缘——TTFB 最低、失效 API 现成，成本按调用计费且绑定平台，重 SEO 页值；纯 SSR 永远新鲜、无缓存 bug，为每次 PV 付渲染 CPU——个性化页/库存价格类"错一秒都算错"的才配。决策树：内容变频率>发布节奏→prerender 出局；含用户态→三档缓存全出局（缓存击穿=安全事故，呼应 cookie-session）；剩下按"平台绑定成本 vs 源端成本"在 isr/swr/SSR 里选。加分句：给出"同一页面不同数据新鲜度要求"的拆法——外壳 swr+个性化块客户端现取，比整页二选一更工程化。

**来源**：Nuxt/Vercel 官方缓存文档；InfoQ《内容站的缓存分层治理》。

### D5.  上线后发现 routeRules 配的缓存在本地 dev 完全看不出问题，到预发才炸，缓存类配置的正确验证链路？

**答**：根因：dev 默认旁路缓存（swr/isr 不参与，routeRules 的 redirect/rewrite 生效但缓存头不落地行为不同步），"本地好使"对缓存策略零证明力。验证链路四层：① 构建产物级——nuxt build 后 nitro 输出里 grep 路由的缓存配置（构建期可断言"这条规则真被解析了"）；② 本地 preview 级——nitro 的 node-server 跑 build 产物，swr 头与第二次请求命中可验（CI 里 curl -D 断言 age/x-nitro-cache 头，这是最便宜的自动化点）；③ 平台级——isr 依赖平台的失效 API/缓存层，预发环境跑真实部署，验收脚本打同一 URL 两次比对 age、调失效接口后验证立刻回源；④ 失效演练——内容更新后实测"多久全网可见新页"，这个 SLA 数字要测出来而不是写出来。组织规矩：routeRules 的变更按架构变更评审（它改的是数据新鲜度契约），PR 必须附"哪一层验证覆盖了这条规则"。

**来源**：掘金《我为什么给缓存策略单开一个 e2e》；SegmentFault《Nuxt 本地与生产行为差异清单》。

### D6.  为什么"带登录态的页面"不能简单上 CDN/缓存？框架给的 ssr/isr 与鉴权交叉的坑有哪些？

**答**：原则：缓存键不含身份，响应就可能串身份。Nuxt 侧具体坑：① routeRules 的 swr/isr 缓存的是"整页 HTML + payload"，payload 里可能带 useFetch 的用户数据——未登页缓存被下一个用户命中=数据泄露级事故；② SSR 页读 cookie 渲染，但缓存在边缘/服务端时 cookie 已被"烘进"静态产物——Nuxt 的做法是带 Set-Cookie 的响应默认不缓存，但你手写 Cache-Control 或用 proxy 绕过 nitro 判断就裸奔；③ middleware 的 navigateTo 重定向若被缓存，未登录用户会收到"302 到登录页"的缓存版，登录后重进还是 302（Vary 没配）；④ ssr:false 的 SPA 壳本身可缓存（无用户数据），但别顺手把同前缀的 API 也 proxy 缓存了。工程规矩：给缓存评审加一问"这条路由的响应里有没有任何用户维度"；鉴权页想加速只能走"壳缓存+数据客户端取+接口层按身份缓存"的拆分，没有第二条路。

**来源**：OWASP 缓存安全备忘；知乎《一次把用户仪表盘缓存住的血案》。
