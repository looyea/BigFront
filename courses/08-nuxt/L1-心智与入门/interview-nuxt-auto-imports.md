...
### D4. "自动导入让代码里有看不见的东西"——怎么向反感的资深工程师解释，并给出团队护栏？

**答**：解释三论：① 它不是隐藏依赖而是"由编译器维护的 import 表"——产物与显式 import 完全一致，view-source 构建产物即可证伪"看不见"；② 它删掉的是样板（每文件顶十行 import）不是信息——IDE hover/跳转/go-to-def 仍然在（prepare 之后），审查面没变；③ 类比已被接受的先例：JSX 自动 runtime、Go 标准库工具函数、Vue 模板里的全局组件。护栏：冲突纪律（composable 按 <域> 前缀命名、撞名显式 import）、ESLint no-undef 关闭但 no-restricted-syntax 管住滥用、utils 目录禁重依赖（它会被"看不见"地拖进每个使用者）、README 列"哪些目录参与自动导入"。收口句："自动导入改变的是书写成本，不是依赖关系——前提是团队把它当契约管而不是当魔法用。"

**来源**：Nuxt 官方 Auto-imports 文档与社区争论；知乎《自动导入是倒退吗》。

### D5.  自动导入对包体积与 tree-shaking 的真实影响？哪些"看不见的引用"最常在 audit 里翻车？

**答**：真相分两半：自动导入本身按具名符号注入，未使用的导出不会进包（tree-shaking 与显式 import 等价）；翻车发生在"符号背后的图"——app/utils 一个被自动导入的函数顶层 import 了 dayjs/axios，引用者看不见这条边，审计时按页面找依赖就漏；更狠的是 Vue 生态包缺 sideEffects 标记或内部动态引用导致摇不动。高发雷区：① utils 塞重依赖（被自动导入放大成"人人都引"）；② 大 barrel index 导出一切；③ 模块注册的 autoImports 指向整包（imports 里把 lodash-es 整包全量暴露这种写法）。治理：rollup-plugin-visualizer（nuxt hook 注入）看 client chunk、"谁引了这个符号"反查用 grep 构建产物而非源码；规矩前置：utils/composables 只引轻依赖、重能力放显式 import+动态加载。

**来源**：Nuxt 官方 Bundle Analysis；掘金《一次 utils 顶层 import dayjs 引发的全员体积膨胀》。

### D6.  从架构角度评价"隐式上下文"（自动导入、全局注册、约定式）的利弊边界？

**答**：隐式的收益只有一种：消灭"重复且无决策"的样板（约定目录的 import、全局组件注册、文件路由）——注意定语：重复+无决策。代价有三类：① 可发现性——新人必须学"魔法目录表"（app/composables 为什么不用 import），学习曲线从代码转移到约定；② 可审查性 diff 看不到依赖边（上面题的雷区）；③ 冲突与仲裁——两个来源提供同名符号时框架策略成为隐性架构决策。边界判据：影响"运行位置/数据归属/缓存行为"的永远要显式（definePageMeta、runtimeConfig 声明、routeRules 评审），只影响"书写"的可以隐式。Nuxt 的取舍算克制：范围限定约定目录+可被显式覆盖+类型生成可见。十年周期律提醒：约定越多，迁移越贵——当年 AngularJS 全局注册、CRA 黑盒配置的债，都会在未来某个"想换框架"的时刻集中还。

**来源**：InfoQ《约定优于配置的代价》；SegmentFault《框架魔法与显式代码的十年周期》。
