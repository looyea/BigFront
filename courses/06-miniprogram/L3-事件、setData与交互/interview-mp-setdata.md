...
> 共 15 题，覆盖 A 原理机制 / B 性能优化 / C 与 React/Vue 对照 / D 实战场景。
...
### 13.  setData 的"合并"到底是深合并还是浅覆盖？对数组、对象、路径写法分别意味着什么？

setData 是"按顶层 key 做合并 + 值整体替换"，不是递归深合并：给 `obj` 传一个新对象会整体替换 obj，不会把你没写的子字段保留下来；想只改子字段必须用路径写法 `obj.a` 或直接传完整新对象。数组同理——传新数组即整体替换，要改某项用 `arr[2].done` 路径追加/修改，否则全量覆盖。路径 setData 有两大价值：① 只传变化的那小块，序列化体积小；② 渲染层能据此做局部 diff。坑：路径里的下标可以是动态变量拼出来的（`['list['+idx+'].done']: true`），但整段 key 必须是字符串；不要用 setData 去改"你没在 data 里声明过的字段"期望它响应式出现——小程序无依赖收集，只有你显式 setData 的部分才会通知渲染层。

微信官方文档《setData 性能注意》；掘金《setData 千次调用优化实录：从 800ms 到 120ms》

### 14.  倒计时每秒刷新、体验评分报"setData 过频"，你会怎么改？给出分层的优化梯度。

先问"每秒都变的是整块还是局部"。梯度：① 只 setData 变化的那个字段（`{ countDown: s }`），别连带周围数据；② 若倒计时只在一处文本，用 wxs 或渲染层格式化把"剩余毫秒→文案"下沉，逻辑层每秒只推一个数字甚至不必；③ 降低更新频率——秒级够用就别用毫秒级 setData；④ 合并同帧其它 setData；⑤ 页面隐藏（onHide）时暂停 setInterval、onShow 再起，避免后台页空转 set；⑥ 极端高频（如动画帧）改用 CSS 动画或渲染层方案（wxs 手势/transition）而非 JS setData。本质：把"高频小变化的计算"移出跨线程 setData 主路径。这与 React 里把高频 setState 拆到 ref/动画、Vue 里避免整表重渲染同理。

SegmentFault《setData 是同步还是异步、返回值为什么拿不到新数据》；知乎《小程序脏数据检查与路径更新》

### 15.  小程序"没有响应式"这句话准确吗？它和 Vue 的响应式、React 的 setState 在"数据→视图"上的本质差异是什么？

基本准确：小程序不会自动侦测 `this.data.x = 1`（改了不通知视图，因为没有 getter/setter 依赖收集，也没有不可变比较），你必须显式 setData 才把变更推给渲染层。对比：Vue 用 Proxy 依赖收集，改响应式属性即自动触发更新，心智是"改状态=改视图"；React 用 setState + 不可变 + 重渲染+diff，心智是"提交新状态、框架重算 UI"。小程序介于两者之间但更"手动"：像 React 一样要显式 set，又像没有 React 的自动重渲染——你 set 什么它才 diff 什么，因此"路径 setData"既是性能手段也是正确性手段。后果差异：① 小程序没有 computed/watch，派生值要么逻辑层算好塞 data、要么 wxs；② 批量更新需自己合并；③ 异步渲染（跨线程）而非同步渲染。理解这点才能解释"为什么直接赋值视图不动""为什么 setData 大对象就卡"。

CSDN《列表长页面 setData 分页+diff 更新方案》；InfoQ《 setData 传输 stringify 开销的度量方法》
