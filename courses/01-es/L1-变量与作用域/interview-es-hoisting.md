...
**13）规范里有「hoisting」这个词吗？用规范术语讲讲提升到底发生了什么。**
- 参考要点：规范没有 hoisting，它是教学比喻。真实机制：执行上下文创建时初始化词法环境——var 声明在 Variable Record 里直接建槽并填 undefined；let/const 在 Declarative Record 里建槽但**不初始化**（TDZ）；函数声明则在实例化阶段就把函数对象填好。三种「提升差异」其实是三种初始化时机差异。
- 来源：ECMAScript 规范 4.1《Executable Code and Execution Contexts》与 Environment Records 一节的转述；kangax 博文。

---

**14）`class B extends A` 里父类表达式的 TDZ 存活期有多长？class 自身的名字在类体内能用吗？**
- 参考要点：extends 后的表达式在 class 定义语句**执行时**就求值（不会推迟到 new），所以 A 在 TDZ 就直接报错；而 class 自身的名字绑定在类体内从第一行起就可用（静态方法里自递归很常见），类外则必须等语句执行完——一个「内部先行、外部后置」的不对称设计。
- 来源：MDN《class extends》《Class declarations > Description》；TC39 Classes FAQ 的转述。

---

**15）函数声明的整体提升，能不能被正向利用？ESLint 怎么划线？**
- 参考要点：可以：它让「入口/注册代码在上、工具函数在下」的自顶向下阅读顺序可行，递归 helper 也不需要前向声明。划线方式：`no-use-before-define` 配 `{ functions: false }` 只放过函数声明，var/let/class 一律拦截；TypeScript 再做一层编译期保险。立场：只用函数声明这一种提升，其余都当 bug。
- 来源：ESLint 文档《no-use-before-define》选项说明；主流 style guide 通行约定的转述。
