...
**13）`new Function('return this')()` 创建的函数，它的作用域链长什么样？工程上为什么罕用？**
- 参考要点：Function 构造器产出的函数只挂**全局**作用域，看不到调用处的词法环境——它相当于动态编译的全局代码；也因此被 CSP 直接拒绝、打包器无法静态分析。**追问**：`eval` 与 `new Function` 在作用域上的区别（前者继承当前作用域，后者不继承）。
- 来源：MDN《Function 构造器与函数声明的区别》；web.dev《Learn JavaScript: Scopes and closures》的转述。

---

**14）ESM 模块顶层的 `this` 为什么是 `undefined`，而经典脚本里是 `window`？**
- 参考要点：模块代码**恒为严格模式**，且规范把模块顶层 this 定义为 undefined；经典脚本 sloppy 模式下顶层 this 指向 globalThis。推论：不能靠模块顶层 this 偷拿全局对象，要显式写 `globalThis`。这也是「模块作用域天然隔离」的一部分。
- 来源：MDN《JavaScript modules > Strict mode》；Node.js ESM 文档关于顶层 this 的说明的转述。

---

**15）`typeof` 为什么救不了 TDZ？它的设计初衷是应对什么场景？**
- 参考要点：typeof 的「不报错豁免」只针对**从未声明**的标识符；TDZ 里的 let/const 绑定已存在，任何读取（含 typeof）都抛 ReferenceError。它的本意是探测宿主环境里可能不存在的名字（`typeof module`、`typeof Vue`），而不是绕开暂时性死区。
- 来源：MDN《typeof》；2ality《JavaScript’s Temporal Dead Zone》的转述。
