...
**13）ES2019 为什么强制 sort 稳定？不稳定的旧实现踩过什么坑。**
- 参考要点：稳定指相等元素保持原相对序；旧 V8 对长数组用快排，相等键会换位——多字段排序（先 sort 次键再 sort 主键）的惯用法直接失效；ES2019 后规范强制稳定，V8 换 TimSort。**追问**：多级排序不传复合比较器、只靠连排为什么现在可以了。
- 来源：Mozilla Hacks《Array.prototype.sort() now stable》；TC39 sort 稳定性提案的转述。

---

**14）遍历方法对洞（hole）的处理有什么不同？`[,,1].map/forEach/filter` 结果各自怎样。**
- 参考要点：map/forEach/filter 用 HasProperty 跳过洞（map 保留洞）；reduce 无初值遇空数组抛错、有初值时洞当 undefined 读；扩展/Array.from/for-of 把洞当 undefined 变密。工程建议：造数组用 Array.from({length})，别用 new Array(n) 直接 map。
- 来源：MDN《数组与稀疏数组（holes）》；ECMA-262 Indexed Collection 算法的转述。

---

**15）flatMap / flat 的常见误用：为什么「把对象数组拍平」用 flatMap 会拍坏。**
- 参考要点：flatMap 只 map 后 flat(1)，map 回调返回对象时没拍（对象不是数组）；但如果元素本身是数组（嵌套列表）会被拍一层，破坏结构。拍数字列表用 flat(Infinity)；转换+拍平才用 flatMap；用前想清楚数据结构。**追问**：flatMap 过滤兼映射的惯用法（返回 [] 或 [x]）。
- 来源：MDN《Array.prototype.flatMap / flat》；Flat Stage Proposal 动机章节的转述。
