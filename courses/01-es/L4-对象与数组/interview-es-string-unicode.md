...
**13）length、for-of、[...str]、Array.from 四者在含 emoji 字符串上分别数的是什么？**
- 参考要点：length 数 UTF-16 码元（'😀'.length=2）；for-of / [...str] / Array.from 都走迭代器、按码点分簇（'😀' 算 1）——但码点≠字素簇，带组合符/ZWJ emoji 还会拆多段。**追问**：为什么 [..'á'].length 可能是 2。
- 来源：MDN《String.length / 迭代协议》；Mathias Bynens 关于 Unicode 演讲的转述。

---

**14）为什么两个看起来一模一样的字符串 `===` 为 false？给出两种成因与统一方法。**
- 参考要点：成因一 NFC/NFD（á 单码点 vs a+组合重音）；成因二全角/半角或同形异码点字符。统一：normalize('NFC') 后再比；显示宽度用 Intl.Segmenter；排序用 localeCompare。**追问**：为什么数据库存前要 normalize。
- 来源：Unicode UAX #15 规范化形式；MDN String.prototype.normalize 的转述。

---

**15）Intl.Segmenter 比 split('')/for-of 强在哪？什么时候没必要用。**
- 参考要点：Segmenter 按字素簇（grapheme）分，把一个 emoji/组合字当整体，for-of 只到码点层；UI 截断、计数、光标移动需它；纯 ASCII 或无组合符时 for-of 已够且开销小，Segmenter 构造成本高不适合热路径。**追问**：为什么家庭 emoji 👨‍👩‍👧‍👦 长度能到 11。
- 来源：MDN Intl.Segmenter；Unicode 字素簇分段（UAX #29）的转述。
