...
**13）面试原题：实现 `add(2)(3)(4)` 输出 9——自动柯里化 + 隐式终止怎么写？**
- 参考要点：用闭包累积参数，给返回函数挂 `toString`/`valueOf`，在被当数字/字符串用时触发求和；与「指定 arity 的 curry」区别在于终止条件是隐式（类型转换）还是显式（fn.length）。**追问**：为什么面试爱问它（考闭包 + 原始值转换双知识点）。
- 来源：StackOverflow《Implement add(2)(3)(4) in JavaScript》高票答案；JS 隐式转换博文的转述。

---

**14）柯里化什么时候值得用，什么时候是噪音？给工程立场。**
- 参考要点：值得：给 map/filter 造一元配置器、声明式管道、命名空间 logger 避免重复传参；噪音：嵌套箭头可读性差、TS 对多态推断需类型神术、调试堆栈难读。结论：小 arity（2-3）配置场景用 lodash.curry 即可，不本身重造。
- 来源：lodash 文档《curry》适用说明；Scott Wlaschin《Currying and Partial Application》的转述。

---

**15）柯里化与闭包是什么关系？一句话讲清。**
- 参考要点：柯里化每一步返回的新函数，靠**闭包**记住已固定的前缀参数——柯里化是闭包的一个应用案例，闭包不依赖柯里化存在。**追问**：`f(a)(b)(c)` 的参数存在哪（每层一个词法环境逐层嵌套，而非展平共享）。
- 来源：You Don't Know JS 闭包卷；MDN《Currying》的转述。
