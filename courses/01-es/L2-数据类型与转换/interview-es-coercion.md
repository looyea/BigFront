...
**13）`null == undefined` 为 true、`null == 0` 为 false、`null >= 0` 为 true——规范为什么这么设计？**
- 参考要点：== 算法开头两步把 null/undefined 锁死为「只互等」，避免空值参与数值比较被误判相等；关系比较没有短路，统一 ToNumber 把 null→0，于是出现「不等但大于等于」的不对称。设计意图：空值的「数值含义」只允许出现在排序比较，不允许被说成等于 0。工程解法：用 === 与显式判空绕开整个泥潭。
- 来源：ECMAScript 规范 Abstract Equality Comparison / Relational Comparison 小节；Dr. Axel Rauschmayer《The mysteries of JavaScript's null》的转述。

---

**14）顶层 `{} + []` 经常得到让人意外的结果，和 `({}) + []` 差别在哪？**
- 参考要点：语句位置上 `{} ` 先被解析成**空块**，剩下 `+[]` 是一元正号 → `Number([])` → 0；加括号后才是对象字面量参与加法，走 ToPrimitive 得 '[object Object]0' 再拼接。教训：解析器按语法位置而非直觉分派，对象字面量出现在行首要避免。
- 来源：MDN《运算符优先级 > 对象字面量与代码块的歧义》说明；StackOverflow 同名高票问题的转述。

---

**15）团队禁用隐式转换后，空值判断和 truthy 判断统一怎么写？**
- 参考要点：等值一律 ===，转换写显式（Number()/String()/Boolean()）；null/undefined 双检用 `x == null`（eqeqeq 的 smart 模式）或拆成两个 ===；truthy 只留给真正布尔语义；可选链/空值合并（`??`、`?.`）是规范背书的新一代显式简写；上 TS 后 string+number 在编译期就被拦，隐式转换空间大幅压缩。
- 来源：ESLint《eqeqeq》文档 smart 模式说明；TypeScript Handbook《Nullish Coalescing》的转述。
