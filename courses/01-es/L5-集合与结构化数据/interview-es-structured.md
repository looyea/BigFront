...
**13）JSON 大法深拷贝与 structuredClone 相比，除了不支持 Map/Set/Date，还有哪些会翻车？**
- 参考要点：JSON 丢 undefined/函数/Symbol、Date 变字符串、BigInt 直接抛错、循环引用抛错、正则变空对象、稀疏数组洞变 null、原型与方法全没；structuredClone 保留内置类型与循环，但同样不保留原型（instanceof 失效）。**追问**：为什么两者都不能克隆函数。
- 来源：MDN《structuredClone》；JavaScript 社区「JSON deep copy pitfalls」类文章的转述。

---

**14）structuredClone 不保留原型链，克隆 class 实例会得到什么？工程上怎么保留类型。**
- 参考要点：只拷自有可枚举数据属性，得到普通对象，原型与方法丢失、instanceof 失败；要保留类型需自己实现 toJSON/fromJSON、显式 new Class(clone)、或用 structuredClone 后 setPrototypeOf。**追问**：为什么 Web 标准不拷贝原型（安全/跨域隔离）。
- 来源：MDN《structuredClone > 描述（不保留原型链）》；HTML 规范 StructuredCloneAlgo 的转述。

---

**15）什么场景该用 structuredClone、什么该用浅拷贝或 immer？**
- 参考要点：跨 Worker/postMessage、快照/undo、隔离可变依赖用 structuredClone（大对象有拷贝开销）；Redux/不可变更新用 immer（produce 自动结构共享、不全量拷贝）；只需隔离一层用展开/浅拷。热路径全量 structuredClone 会明显卡颁。
- 来源：MDN《structuredClone》；immer 官方 README（structural sharing）的转述。
